 ...
 * Ported from the browser-side OnsetDetector and analysis engines.
...
// We only need spectral centroid and band energy at frame level, not a full FFT.
// This runs on 512-sample frames and is fast enough for offline analysis.

function computeSpectrum(frame: Float32Array, sampleRate: number): Float32Array {
...
// Lightweight: compute HFC + band energy without full DFT —
// instead use a bank of IIR-approximate band energies via RMS in windows.
// For BPM and onset detection we only need the broadband energy envelope.
...
function estimateBpm(onsetTimesMs: number[]): number | null {
  ...
  for (const ioi of iois) {
    ...
    // Also include octave candidates
...
export function analyzePcm(samples: Float32Array, sampleRate: number): AudioAnalysisReport {
  ...
  // ── Onset detection (HFC-based, port of OnsetDetector.ts) ─────────────────
  ...
  if (isSilent) {
    parts.push('Audio is silent — nothing was captured or the organism was not running.')
  } else {
    ...
    if (estimatedBpm) {
      parts.push(`Rhythm: estimated ${estimatedBpm} BPM, ${onsetCount(onsetTimesMs)} onsets detected.`)
