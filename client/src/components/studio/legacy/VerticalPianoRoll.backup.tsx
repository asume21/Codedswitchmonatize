...
const VerticalPianoRoll: React.FC<VerticalPianoRollProps> = ({
  ...
  const [scaleFilter, setScaleFilter] = useState<string>('chromatic'); // chromatic = all notes
  ...
  const snapToGrid = useCallback((step: number): number => {
    ...
    const snapSteps = snapValue; // 1 = quarter note, 0.5 = eighth, 0.25 = sixteenth
    return Math.round(step / snapSteps) * snapSteps;
  ...
  const copySelected = useCallback(() => {
    ...
    toast({ title: '📋 Copied', description: `${selectedNotes.length} note${selectedNotes.length === 1 ? '' : 's'} copied to clipboard` });
  }, [selectedNoteIds, activeNotes, toast]);

  // Paste notes from clipboard
  const pasteNotes = useCallback(() => {
    if (clipboard.length === 0) return;
    
    // Find the earliest step in clipboard
    const minStep = Math.min(...clipboard.map(n => n.step));
    
    // Paste at current step, offset by the difference
  ...
  }, [clipboard, currentStep, activeNotes, onNotesChange, addToHistory, snapToGrid, toast]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        if (key === 'v') { e.preventDefault(); pasteNotes(); return; }
      }
  ...
  }, [activeKeys, currentOctave, noteLength, selectedNoteIds, isPlaying, currentStep, tracks, selectedTrack, undo, redo, duplicateSelected, deleteSelected, copySelected, pasteNotes, onNotesChange, addToHistory, toast]);
  ...
  const handleGridMouseDown = (event: React.MouseEvent<HTMLDivElement>) => {
    if (!activeTrack || !gridContentRef.current) return;

    // Use the grid content's bounding rect directly (the element we clicked on)
    const gridRect = gridContentRef.current.getBoundingClientRect();

    // Calculate position relative to the grid content itself
    const offsetX = event.clientX - gridRect.left;
    const offsetY = event.clientY - gridRect.top;

    const rawStep = Math.floor(offsetX / STEP_WIDTH);
    ...
    // Check if clicking within an existing note's range (use RAW step, not snapped)
    const existing = activeNotes.find(note => {
      if (note.note !== noteName || note.octave !== octave) return false;
      const length = note.length ?? 1;
      return rawStep >= note.step && rawStep < note.step + length;
    });

    // Handle based on selected tool
    if (pianoRollTool === 'erase') {
      // Erase mode - delete note if exists
      if (existing) {
        const updatedNotes = activeNotes.filter(n => n.id !== existing.id);
        onNotesChange(updatedNotes);
        addToHistory(updatedNotes);
        toast({ title: '🗑️ Note Erased', description: `${existing.note}${existing.octave} removed` });
      }
    } else if (pianoRollTool === 'select') {
      // Select mode - just select the note
      if (existing) {
        setSelectedNoteId(existing.id);
      }
    } else {
      // Draw mode - default behavior
      if (existing) {
        setSelectedNoteId(existing.id);
      } else {
        // Only snap when creating NEW notes
        const step = snapToGrid(Math.min(Math.max(rawStep, 0), STEPS - 1));
  ...
  const getNotePosition = (note: Note) => {
    ...
    // Calculate note length, ensuring it doesn't overlap with next note on same row
    let noteLength = note.length ?? 1;
    
    // Find any notes on the same row (same pitch) that come after this note
    const notesOnSameRow = activeNotes.filter(n => {
      if (n.id === note.id) return false;
      const nMidiValue = (n.octave + 1) * 12 + NOTE_NAMES.indexOf(n.note);
      return nMidiValue === midiValue && n.step > note.step;
    });
    
    // Find the closest note after this one
    if (notesOnSameRow.length > 0) {
      const nextNote = notesOnSameRow.reduce((closest, n) => 
        n.step < closest.step ? n : closest
      );
      
      // Limit this note's length to not overlap with the next note
    ...
    return {
      ...
      width: Math.max(1, noteLength) * STEP_WIDTH - 2, // -2px gap between notes
    };
  };
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between p-3 bg-slate-900 text-white border-b border-slate-700">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-1 border-l border-slate-600 pl-4">
              <Button
                ...
                className={pianoRollTool === 'draw' ? 'bg-blue-600' : ''}
                title="Draw notes"
              >
                ✏️ Draw
              </Button>
              <Button
                ...
                className={pianoRollTool === 'select' ? 'bg-blue-600' : ''}
                title="Select notes"
              >
                ☝️ Select
              </Button>
              <Button
                ...
                className={pianoRollTool === 'erase' ? 'bg-red-600' : ''}
                title="Erase notes"
              >
                🗑️ Erase
              </Button>
            ...
            <div className="flex items-center space-x-2 border-l border-slate-600 pl-4">
              <span className="text-sm text-slate-200">Scale:</span>
            ...
            <Button
              ...
              onClick={() => {
                ...
                toast({
                  title: chordMode ? '🎹 Chord Mode Off' : '🎹 Chord Mode On',
                  description: chordMode ? 'Single notes mode' : 'Click to place chords'
                });
              }}
              className={chordMode ? 'bg-green-600 hover:bg-green-700' : ''}
            >
              🎹 Chord Builder
            </Button>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex items-center space-x-2 px-2 border-l border-slate-600">
              <Button
                variant={snapEnabled ? "default" : "outline"}
                size="sm"
                onClick={() => setSnapEnabled(!snapEnabled)}
                className={snapEnabled ? "bg-blue-600 hover:bg-blue-700" : ""}
                title="Toggle snap to grid"
              >
                🧲 Snap
              </Button>
              <Select value={String(snapValue)} onValueChange={(v) => setSnapValue(Number(v))}>
                <SelectTrigger className="w-20 h-8 bg-slate-800 border-slate-600">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            {/* Velocity Editor Toggle */}
            <Button
              variant={showVelocityEditor ? "default" : "outline"}
              size="sm"
              onClick={() => setShowVelocityEditor(!showVelocityEditor)}
              className={showVelocityEditor ? "bg-purple-600 hover:bg-purple-700" : ""}
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-slate-900 border-b border-gray-200 dark:border-gray-800">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 border-l border-gray-300 dark:border-gray-700 pl-4">
                <span className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Velocity:</span>
                <span className="text-sm font-bold text-gray-800 dark:text-white min-w-[2rem]">
                  {selectedNote.velocity ?? 100}
                </span>
                <Slider
                  className="w-32"
                  value={[selectedNote.velocity ?? 100]}
                  onValueChange={([value]) => updateNoteVelocity(selectedNote.id, value)}
                  min={1}
                  max={127}
                  step={1}
                />
        ...
        <div className="flex-1 relative bg-gray-50 dark:bg-slate-950">
          ...
          <div className="ml-16 h-full overflow-auto" ref={gridWrapperRef}>
            ...
            >
              ...
              {/* Grid lines - Measure markers (every 4 steps) and beat markers */}
              {Array.from({ length: STEPS + 1 }).map((_, index) => {
                const isMeasure = index % 4 === 0;
                return (
                  <div key={`step-line-${index}`}>
                    <div
                      className={`absolute top-0 bottom-0 ${
                        isMeasure 
                          ? 'border-l-2 border-blue-300/80 dark:border-blue-700/60' 
                          : 'border-l border-gray-300/40 dark:border-gray-700/30'
                      }`}
                      style={{ left: index * STEP_WIDTH }}
                    />
                    {isMeasure && (
                      <div 
                        className="absolute top-0 text-[10px] text-blue-500 dark:text-blue-400 font-semibold px-1 bg-gray-100/80 dark:bg-slate-900/80 rounded"
                        style={{ left: index * STEP_WIDTH + 2, transform: 'translateY(-100%)' }}
                      >
                        {index / 4 + 1}
                      </div>
                    )}
              ...
              {activeNotes.map((note) => {
                ...
                const velocityHeight = (velocity / 127) * 100; // Percentage
                ...
                return (
                  <React.Fragment key={note.id}>
                    ...
                    {/* Velocity bar (shown when velocity editor is enabled) */}
                    {showVelocityEditor && (
                      <div
                        className="absolute bottom-0 bg-purple-500/40 border-t border-purple-400/60 pointer-events-none"
                        style={{
                          left: position.left,
