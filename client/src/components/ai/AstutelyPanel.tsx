...
export default function AstutelyPanel({ onClose, onGenerated }: AstutelyPanelProps) {
  ...
  const handleGenerateRealAudio = async (styleName?: string) => {
    ...
    try {
      ...
      toast({
        title: 'Audio Ready',
        description: `Generated by ${result.provider}`,
      });

      // Do NOT auto-play; wait for user click to avoid double playback
    } catch (error: any) {
  ...
  const handleGenerate = async () => {
    ...
    toast({ title: 'Astutely Activated', description: `Creating ${selectedStyle.name} beat with professional audio...` });

    // Simulate progress for UX
    const interval = setInterval(() => {
      setProgress(p => {
        const newProgress = Math.min(p + 10, 90);
        // Emit progress events
        if (newProgress < 30) {
          dispatchAstutelyEvent('generation-progress', { step: 'planning', progress: newProgress });
        } else if (newProgress < 60) {
          dispatchAstutelyEvent('generation-progress', { step: 'generating-pattern', progress: newProgress });
        } else if (newProgress < 85) {
          dispatchAstutelyEvent('generation-progress', { step: 'generating-audio', progress: newProgress });
        } else {
    ...
    try {
      ...
      setGeneratedResult(completeResult.pattern);
      
      // Store audio result
      setGeneratedAudioUrl(completeResult.audio.audioUrl);
      setAudioProvider(completeResult.audio.provider);
      ...
      if (completeResult.pattern.meta?.usedFallback || completeResult.pattern.isFallback) {
        toast({
          title: 'Using Offline Pattern',
          description: 'Pattern generated offline. Audio generated via ' + completeResult.audio.provider,
        });
      }
      ...
      if (onGenerated) {
        onGenerated(completeResult.pattern);
      }
          ...
          {generatedAudioUrl && (
            <div className="mb-4 p-4 rounded-xl bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 border border-emerald-500/40">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-emerald-200/80">Rendered Audio</p>
                ...
                <button
                  onClick={() => {
                    window.dispatchEvent(new CustomEvent('importToMultiTrack', {
                      detail: {
                        type: 'audio',
                        name: `${selectedStyle.name || 'Track'}`,
                        audioUrl: generatedAudioUrl,
                      }
                    }));
                    toast({ title: 'Added to Multi-Track', description: 'Use main transport to play.' });
                  }}
                  className="p-3 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 transition-all"
                  title="Add to Multi-Track"
                >
          ...
          {generatedResult && (
            <div className="mt-2 p-4 rounded-xl bg-green-500/20 border border-green-500/40">
              ...
              <button
                onClick={async () => {
                  try {
                    await renderPatternToStems(generatedResult);
                    toast({ title: 'Stems Rendered', description: 'Drums, bass, chords, and melody added to Multi-Track as audio.' });
                  } catch (error) {
                    toast({ title: 'Render Failed', description: 'Could not render pattern to audio stems.', variant: 'destructive' });
                  }
                }}
                className="mt-3 w-full py-2.5 rounded-lg bg-cyan-500/30 hover:bg-cyan-500/40 text-cyan-300 border border-cyan-500/40 font-semibold text-sm transition-all flex items-center justify-center gap-2"
              >
                <Disc3 className="w-4 h-4" />
                Render to Audio Stems
              </button>
            </div>
          )}
