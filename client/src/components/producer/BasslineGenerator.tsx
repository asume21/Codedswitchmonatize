...
export function BasslineGenerator({
  ...
  const [bassPattern, setBassPattern] = useState<string[]>(
    new Array(16).fill(""),
  );
  const [bassLengths, setBassLengths] = useState<number[]>(
  ...
  const playBassNote = async (note: string, duration: number = 0.5) => {
    ...
    gainNode.gain.linearRampToValueAtTime(
      gainNode.gain.value * sustainLevel,
    ...
    filter.frequency.exponentialRampToValueAtTime(
      400,
      ctx.currentTime + Math.min(duration * 0.5, 0.3),
    );
  ...
  useEffect(() => {
    const masterPlayHandler = (event: any) => {
      ...
      console.log(
        `🎸 Bass received master control: ${shouldPlay ? "PLAY" : "STOP"}`,
      );
    ...
    <Card className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div>
          ...
          <Select
            value={scale}
            onValueChange={(value: keyof typeof HIP_HOP_SCALES) =>
              setScale(value)
            }
          >
        ...
        <div>
          <label className="text-sm font-medium">
            Default Length: {defaultDuration[0]}s
          </label>
          <Slider
          ...
          <div className="text-xs text-gray-500 mt-1">
            {defaultDuration[0] < 0.3
              ? "Staccato"
              : defaultDuration[0] < 0.8
                ? "Normal"
                : "Sustained"}
          </div>
        </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <Button
          onClick={generateClassicBassline}
          variant="outline"
          data-testid="button-classic-bass"
        >
          Classic Hip-Hop
        </Button>

        <Button
          onClick={generate808Bassline}
          variant="outline"
          data-testid="button-808-bass"
        >
          808 Style
        </Button>
          ...
          <div className="flex gap-1">
            {bassPattern.map((note, stepIndex) => (
              <div key={stepIndex} className="flex flex-col items-center gap-1">
                <div className="text-xs text-center w-16">{stepIndex + 1}</div>

                <div className="flex flex-col gap-1">
                  <Select
                    value={note || "-"}
                    onValueChange={(value) =>
                      setStepNote(stepIndex, value === "-" ? "" : value)
                    }
                  >
                    <SelectTrigger
                  ...
                  {note && note !== "-" && (
                    <div className="w-16">
                      <Slider
                        ...
                        onValueChange={(value) =>
                          setStepDuration(stepIndex, value[0])
                        }
                        min={0.1}
                        max={2.0}
                        step={0.1}
