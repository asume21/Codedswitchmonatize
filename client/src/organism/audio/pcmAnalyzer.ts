/**
 * PCM Audio Analyzer — WebEar-grade signal analysis running client-side.
 *
 * Ported from server/services/mcpAudioAnalysis.ts (the engine behind WebEar's
 * analyze_audio MCP tool). This gives the Organism and Astutely the same
 * quality audio intelligence that Claude Code gets via WebEar — but with
 * zero API calls, running entirely in the browser on raw Float32Array PCM.
 *
 * Used by SelfListenAnalyzer to produce rich SelfListenReports every capture
 * cycle, enabling smarter self-correction and genre-aware mix decisions.
 */
...
export interface PcmAnalysisReport {
  ...
  onsetCount:            number
  onsetTimingStdDevMs:   number   // low = tight groove, high = erratic
...
// ── BPM estimation via onset IOI binning ───────────────────────────────────────
...
export function analyzePcm(samples: Float32Array, sampleRate: number): PcmAnalysisReport {
  ...
  if (isSilent) {
    parts.push('Audio is silent.')
  } else {
    parts.push(`Loudness: ${rmsDb.toFixed(1)} dBFS RMS, peak ${peakDb.toFixed(1)} dBFS.`)

    if (hasClipping) parts.push(`Clipping on ${clippingPercent.toFixed(2)}% of samples.`)
    if (hasDcOffset) parts.push(`DC offset ${dcOffset.toFixed(4)}.`)

    if      (spectralCentroidHz < 800)  parts.push('Tone: very dark / sub-heavy.')
    else if (spectralCentroidHz < 2000) parts.push(`Tone: warm (centroid ${spectralCentroidHz.toFixed(0)} Hz).`)
    else if (spectralCentroidHz < 4000) parts.push(`Tone: balanced (centroid ${spectralCentroidHz.toFixed(0)} Hz).`)
    else                                parts.push(`Tone: bright (centroid ${spectralCentroidHz.toFixed(0)} Hz).`)

    parts.push(`Bands — sub: ${(bandEnergy.sub*100).toFixed(0)}% | bass: ${(bandEnergy.bass*100).toFixed(0)}% | mid: ${(bandEnergy.lowMid*100).toFixed(0)}% | hi-mid: ${(bandEnergy.highMid*100).toFixed(0)}% | high: ${(bandEnergy.high*100).toFixed(0)}%.`)

    if (estimatedBpmValue) {
      parts.push(`${estimatedBpmValue} BPM, ${onsetTimesMs.length} onsets.`)
      if      (onsetTimingStdDevMs < 5)  parts.push('Timing: tight.')
      else if (onsetTimingStdDevMs < 15) parts.push(`Timing: ok (${onsetTimingStdDevMs.toFixed(1)} ms jitter).`)
      else                               parts.push(`Timing: loose (${onsetTimingStdDevMs.toFixed(1)} ms jitter).`)
    } else {
      parts.push('No clear beat detected.')
    }

    if (dynamicRangeDb < 4)       parts.push('Mix sounds crushed — low dynamic range.')
    else if (dynamicRangeDb > 18) parts.push('Very dynamic — could use more compression.')

    if (crestFactor < 2) parts.push('Transients are flattened — over-compressed.')
  }
