...
export class MelodyGenerator extends GeneratorBase {
  ...
  // Band-awareness (GeneratorOrchestrator.syncLeadBusyToChords /
  // maybeAnswerMelodyRest): the current phrase's occupied 16th-grid slots, and
  // the absolute audio-clock time (seconds, comparable to Tone.now()) the most
  // recently triggered note ends — 0 means the melody hasn't played yet.
  ...
  private rebuildPhrase(physics: PhysicsState, _organism: OrganismState): boolean {
    ...
    this.part = new Tone.Part((time, event) => {
      ...
      // Band-awareness: track when THIS note ends (absolute audio-clock time)
      // so GeneratorOrchestrator.maybeAnswerMelodyRest() can detect a real rest.
      try {
        const durSec = Tone.Time(event.dur).toSeconds()
        this.lastNoteEndSec = Math.max(0, time) + durSec
      } catch { /* keep previous value — signal busy / negative time */ }
  ...
  /** Band-awareness: 16th-grid positions (0-15) the current phrase occupies,
   *  so ChordGenerator can dodge the lead the same way it dodges the kick. */
  getBusySlots16ths(): number[] {
    return this.currentPhraseNotes.map((n) => sixteenthPosOf(n.time))
  }
