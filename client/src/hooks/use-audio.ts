...
async function enableIOSAudio(): Promise<void> {
  ...
  try {
    ...
    if (!iOSAudioContext) {
      iOSAudioContext = new (window.AudioContext ||
  ...
  return {
    playMelody: (notes: any[], bpm: string = "120", tracks?: any[]) => {
      ...
      Object.entries(notesByTime).forEach(
        ([startTime, notesAtTime]: [string, any]) => {
          const timeout = setTimeout(
            () => {
              ...
              (notesAtTime as any[]).forEach((note) => {
                if (note && note.note && note.track) {
                  ...
                  console.log(
                    `Playing ${instrument}: ${note.note}${note.octave} for ${note.duration}s (track: ${note.track})`,
                  );
                  playNote(
                    note.note,
                    note.octave || 4,
...
export function useAudio(): UseAudioReturn {
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      // Initialize all audio engines (advanced is our best one)
      ...
      toast({
        ...
        description:
          "Advanced Tone.js, realistic, and synthetic audio engines initialized successfully.",
      });
    } catch (error) {
      ...
      toast({
        ...
        description:
          "Could not initialize audio system. Some features may not work.",
        variant: "destructive",
      });
    }
  }, [toast]);
  ...
  const playNote = useCallback(
    async (
      ...
      try {
        ...
        console.log(
          `Audio Mode Check: useRealisticSounds=${useRealisticSounds}, realisticAudio.isReady()=${realisticAudio.isReady()}`,
        );

        if (useRealisticSounds && realisticAudio.isReady()) {
          // Use realistic sampled instruments
          console.log(
            `Playing REALISTIC ${instrument}: ${noteString}${octave}`,
          );
          ...
          // Use our advanced Tone.js engine (best quality synthesis)
          console.log(
            `Playing ADVANCED TONE.JS ${instrument}: ${noteString}${octave}`,
          );
          try {
            await advancedAudioManager.playNote(
              `${noteString}${octave}`,
              duration.toString(),
            );
          } catch (error) {
            // Fallback to basic synthetic engine
            console.log(
              `Fallback to SYNTHETIC ${instrument}: ${noteString}${octave}`,
            );
  ...
  const playDrumSound = useCallback(
    async (type: string, volume: number = 0.5) => {
      try {
        if (!globalAudioInitialized) {
          await initialize();
        }
        ...
        if (useRealisticSounds && realisticAudio.isReady()) {
          ...
          // Use our advanced Tone.js drum kit (best synthetic drums)
          console.log(`Playing ADVANCED TONE.JS DRUM: ${type}`);
          try {
            // Map common drum types to our advanced drum kit
            const drumMap: { [key: string]: () => void } = {
              kick: () =>
                advancedAudioManager
                  .initialize()
                  .then(() =>
                    (
                      advancedAudioManager as any
              ...
              hihat: () =>
                advancedAudioManager
                  ...
                  .then(() =>
                    (
                      advancedAudioManager as any
                    ).drumKit?.hihat?.triggerAttackRelease(
                      220,
                      "16n",
                      undefined,
                      volume,
            ...
            const drumAction = drumMap[type.toLowerCase()];
            if (drumAction) {
              await drumAction();
            } else {
              // Fallback to basic synthetic drums
  ...
  const toggleRealisticSounds = useCallback(() => {
    setUseRealisticSounds((prev) => {
      const newValue = !prev;
      console.log(
        `🎵 Audio Mode Toggled: ${prev ? "REALISTIC" : "SYNTHETIC"} → ${newValue ? "REALISTIC" : "SYNTHETIC"}`,
      );
      return newValue;
    });
  }, []);
