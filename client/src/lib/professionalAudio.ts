...
export class ProfessionalAudioEngine {
  ...
  private async createDefaultSends(): Promise<void> {
    ...
    // Send 1: Hall Reverb
    const hallSend = await this.createSendReturn(
      "hall",
      "Hall Reverb",
      "reverb",
    );
    ...
    // Send 2: Delay
    const delaySend = await this.createSendReturn(
      "delay",
      "Stereo Delay",
      "delay",
    );
    ...
    // Send 3: Chorus
    const chorusSend = await this.createSendReturn(
      "chorus",
      "Vintage Chorus",
      "chorus",
    );
  ...
  private async createSendReturn(
    id: string,
    name: string,
    effectType: string,
  ): Promise<SendReturn> {
  ...
  private async createEffect(
    type: string,
  ): Promise<{ node: AudioNode; effect: AudioEffect }> {
  ...
  soloChannel(channelId: string, solo: boolean): void {
    ...
    // Handle solo logic
    const soloedChannels = Array.from(this.channels.values()).filter(
      (ch) => ch.solo,
    );
