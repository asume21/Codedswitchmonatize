...
export class RealisticAudioEngine {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(
        "🎵 Realistic audio context created, state:",
      ...
      console.log(
        "🎵 Realistic audio context started, final state:",
        this.audioContext.state,
      );
      ...
      console.log(
        "Realistic audio engine initialized with sampled instruments",
      );
    } catch (error) {
  ...
  private async loadInstruments(instrumentNames: string[]): Promise<void> {
    ...
    const loadPromises = instrumentNames.map(async (name) => {
      ...
      try {
        ...
        console.log(
          `🎵 Loading realistic ${name} instrument (${soundfontName})`,
        );
  ...
  async loadAdditionalInstrument(instrumentName: string): Promise<void> {
    if (
      ...
      !(instrumentName in this.instrumentLibrary) ||
      !this.audioContext
    ) {
      return;
    }
    ...
    try {
      ...
      console.log(
        `Loaded additional realistic ${instrumentName} instrument successfully`,
      );
    } catch (error) {
      console.error(
        `Failed to load additional ${instrumentName} instrument:`,
        error,
      );
    }
  }
  ...
  async playNote(
    ...
    if (!instrumentSampler) {
      console.warn(
        `Realistic instrument ${realInstrument} not available, skipping playback`,
      );
      return;
    }
    ...
    try {
      ...
      const playedNote = instrumentSampler.play(
        noteName,
      ...
      console.log(
        `Playing realistic ${realInstrument}: ${noteName} for ${finalDuration}s`,
      );
    } catch (error) {
      console.error(
        `Error playing realistic note ${note}${octave} on ${realInstrument}:`,
        error,
      );
    }
  }
  ...
  async playDrumSound(drumType: string, velocity: number = 0.7): Promise<void> {
    ...
    if (!this.instruments["drums"]) {
      console.log(
        "🎵 Using synthetic drums for realistic mode (soundfont drums are broken)",
      );
  ...
  private playSyntheticKick(currentTime: number, velocity: number): void {
    ...
    try {
      ...
      kickClickOsc.frequency.exponentialRampToValueAtTime(
        800,
        currentTime + 0.01,
      );
      ...
      kickClickGain.gain.exponentialRampToValueAtTime(
        clickVol,
        currentTime + 0.003,
      ); // Gentle rise
      kickClickGain.gain.exponentialRampToValueAtTime(
  ...
  private playSyntheticSnare(currentTime: number, velocity: number): void {
    ...
    try {
      ...
      snareToneGain.gain.exponentialRampToValueAtTime(
        0.001,
        currentTime + 0.08,
      );
  ...
  private playSyntheticOpenHat(currentTime: number, velocity: number): void {
    ...
    try {
      ...
      openhatGain.gain.exponentialRampToValueAtTime(
        0.001,
        currentTime + duration,
      );
    ...
    try {
      ...
      for (let i = 0; i < bufferSize; i++) {
        const envelope =
          i < bufferSize * 0.1
            ? 1
            : Math.pow(1 - (i - bufferSize * 0.1) / (bufferSize * 0.9), 4);
        data[i] = (Math.random() * 2 - 1) * envelope;
      }
