 ...
 * - Full Songs (ACE-Step worker → MusicGen/Stable Audio via Replicate)
 * - Beats/Melodies (ACE-Step → MusicGen via Replicate)
 * - Sample Packs (ACE-Step → MusicGen Looper via Replicate + Local Fallback)
 */
export class UnifiedMusicService {
   ...
   * 0. ACE-Step (our own RunPod worker — primary, instrumental)
   * 1. MusicGen Large/Stereo on Replicate (high quality fallback)
   * 2. Stable Audio on Replicate (alternative)
   * 3. Basic MusicGen (last resort)
   *
  ...
  async generateFullSong(prompt: string, options: {
    ...
    // Instrumental only here: generateFullSong receives no lyrics, and a vocal
    // request without lyrics would come back as mumble. Lyrics-aware VOCAL songs
    // are generated by the songs.ts route, which sends lyrics to ACE-Step via the
    // ace↔suno bridge. Vocal requests to THIS method fall through to MusicGen.
    if (!vocals) {
    ...
    // ═══ PROVIDER 1: MusicGen Large/Stereo on Replicate ═══
    try {
    ...
    // ═══ PROVIDER 3: Basic MusicGen (last resort) ═══
  ...
  async generateStitchedSong(prompt: string, options: {
    ...
    if (sectionResults.length === 0) {
      ...
      // Fall back to the full provider cascade (ACE-Step → MusicGen → Stable Audio)
