...
export function createKeyRoutes(storage: IStorage) {
  ...
  router.post("/activate", async (req: Request, res: Response) => {
    try {
      ...
      // Check if key exists in database (generated by Stripe payment)
      const userWithKey = await storage.getUserByActivationKey(activationKey);
      
      if (!userWithKey) {
        return res.status(401).json({ 
          message: "Invalid activation key. Please check your email for the correct key after payment." 
        });
      }

      // Mark key as activated if first time
      if (!userWithKey.activatedAt) {
        await storage.activateUserKey(userWithKey.id);
        console.log(`✅ Activation key activated for user ${userWithKey.id}`);
      }

      // Create session
      if (req.session) {
        req.session.userId = userWithKey.id;
      }

      const { password: _, ...userWithoutPassword } = userWithKey;
      return res.json({ 
        message: "Activation successful! Welcome to CodedSwitch Pro",
        user: userWithoutPassword,
        tier: userWithKey.subscriptionTier || 'pro',
        isOwner: false
      });
