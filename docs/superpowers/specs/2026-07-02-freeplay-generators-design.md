...
## 14. The comp plays a FIGURE it commits to (2026-08-04) — continues §11

> "it doesn't sound like a chord, it sounds like a note with no place"
> "the spacing doesn't work … those notes almost seem to just randomly play whenever"

### 14.1 What was measured (not inferred)

A probe built the real comp plan (`buildFreeplayCompPlan`) for a boom-bap verse
and printed the onset slots per bar:

```
bar 0: [0, 3, 14]     bar 2: [0, 3, 5, 14]
bar 1: [0, 3]         bar 3: [0, 3, 5]
```

Identical across four consecutive phrases, and essentially unchanged when the
melody's busy slots move. So the placement is **stable but arbitrary** — the
"keeps changing" hypothesis was wrong.

Map the slots (16 per bar, 4 per beat): 0 = beat 1, **4 = beat 2**, 8 = beat 3,
**12 = beat 4**. `BACKBEAT = new Set([4, 12])` is hard-excluded at every
selection site, and beat 3 never gets picked. So the comp plays the downbeat and
then **never plays a beat again** — what remains are slots 3 and 5, the
sixteenths immediately before and after beat 2. It lands a 16th away from the
strongest beat in the bar, every bar, and never on it.

That is the mechanical consequence of **subtractive placement**: the rhythm is
generated by exclusion (`!BACKBEAT.has(s) && !collides(s) && leadRoom(bar, s)`,
filling `cell.gaps`), so the part is whatever the kick, the backbeat and the
melody didn't want. A rhythm that is a by-product of four avoid-rules cannot
sound intentional regardless of note quality or sample quality.

Separately, the chord has no **anatomy**. `conductor/voicing.ts` returns
`{ bass, inner, guideTones }`. `ChordGenerator` reads only `.inner` (lines 600,
659, 686, 876). **`voicing().bass` has zero call sites in the codebase** — the
engine computes the correct bass note for every chord and nothing has ever
played it. `guideTones` (the 3rd and 7th, the notes that define the chord) are
read only by `MelodyGenerator`. The keys play a middle cluster with no bottom and
no designated top: "a note with no place".

### 14.2 Design

**1. Comp figures, not leftovers.** A small per-style vocabulary of real comp
rhythms. The chord picks ONE per section — seeded like the drum lock
(`drumSectionSeedKey`), so it is stable for the section — and plays it every bar.
Boom-bap: backbeat stabs. Trap: downbeat + a push into the change. Soul/lo-fi:
offbeat 8ths. `BACKBEAT` stops being a prohibition and becomes a **target** for
the styles built on it.

Avoidance demotes to a **tiebreaker**: a figure hit colliding with a kick shifts
one sixteenth or drops for that bar. It never regenerates the rhythm. The figure
survives.

**2. Voicing anatomy.** Play the `bass` note the engine already computes, OR go
deliberately rootless and let the guide tones imply it — a per-style choice, not
an accident. Give the top voice slightly more velocity so it sings.

### 14.3 Acceptance

- Probe test: the figure is identical across bars and phrases; it lands on the
  style's target slots; a kick collision moves ONE hit, not the bar.
- By ear: a soloed chord capture reads as a part someone is playing.

### 14.4 Risk (state it plainly)

This makes the comp MORE repetitive — the opposite direction from the variety
work reverted in `a70e576e`. That is deliberate: the user's model is that a beat
is locked loops and repetition IS the point, and a figure you can predict is what
makes a comp sound intentional. It is also the assumption most likely to be
wrong, so it gets judged by ear before anything is built on top of it.

Scope: chords only. Not melody, not drums. Hook ownership (§15, later) waits
until an instrument can hold a figure at all.

### 14.5 Correction to §13.5

§13.5 said an event added "in the past" of the current loop iteration is "skipped
(harmless)". **That was wrong and it cost a day.** `updatePartEvents` runs on
every rebuild — every 900ms for the bass (`MIN_REBUILD_INTERVAL_MS`) against a
`'4m'` Part (~10s at 96 BPM) — so the schedule was wiped ~10x per loop and a note
only sounded if it happened to fall between two wipes. Measured live: bass
